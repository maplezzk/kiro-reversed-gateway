...
推荐让 Clash 把 Kiro 域名转到本地 CONNECT proxy，这样不需要改 `/etc/hosts`。

启动脚本默认会同时启动：

```text
127.0.0.1:443                   -> HTTPS 网关
127.0.0.1:${CONNECT_PROXY_PORT}  -> Clash 使用的 HTTP CONNECT 代理
```
...
如果你不使用 Clash，也可以选择 `/etc/hosts` 劫持方式：
...
可以构建一个本地菜单栏 App，一键切换 OpenAI 代理、混合模式和官方直连模式。第一版默认使用 Docker 重启服务。
